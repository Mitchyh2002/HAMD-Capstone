...
import Program.DB.Models.master.Modules as Modules
...
def QueryInsertModule(new_module: Modules.Module):
    #If Module Exists Drop it
    existing_modules = Modules.Module.query.filter_by(prefix=new_module.prefix).delete()
...
@blueprint.route('/get')
def get_plugins():
    return Modules.Module.query.filter_by(Modules.Module.status == True).all()
